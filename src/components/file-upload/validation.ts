...
function rejectionMessage(reason: FileValidationReason) {
...
export function validateFiles(
  ...
  selected.forEach((file, index) => {
    const reason: FileValidationReason | undefined = !acceptsFile(file, options.accept)
      ? "type"
    ...
    if (reason) {
      rejected.push({ file, reason, message: rejectionMessage(reason) });
    } else {
      accepted.push(file);
    }
  });
